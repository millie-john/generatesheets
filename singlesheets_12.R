...
#
# Generate 5 sheets, then add labels 1,2,3, and then check for distribution again (no more than 2 in a row and equal distribution)
# Individual sheets are named `Iteration_12_<number>.csv`
...
for (i in 1:5000000) {
  ...
  write.csv(
    x = df_save,
    file = paste0("Iteration_12_", i, ".csv"),
    row.names = FALSE
  )
